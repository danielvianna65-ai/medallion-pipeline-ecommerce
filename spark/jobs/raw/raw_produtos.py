...
if not fs.exists(landing_path):
    print("⚠️ Nenhuma landing encontrada para produtos. Encerrando job.")
    spark.stop()
    exit(0)
...
if not datas_disponiveis:
    print("⚠️ Nenhuma partição dt encontrada na landing de produtos.")
    spark.stop()
    exit(0)
...
input_path = (
    ...
    f"produtos/dt={execution_date}"
)

output_base_path = (
    "hdfs://namenode:8020/data/raw/ecommerce/produtos"
)
...
df_raw = (
    ...
    .select(
        ...
        col("preco").cast("decimal(12,2)"),
        col("ativo").cast("boolean"),
    )
)
...
# Validação mínima
# -----------------------------
df_raw = df_raw.filter(col("id_produto").isNotNull())
...
# -----------------------------
df_raw = df_raw.dropDuplicates(["id_produto"])
...
spark.sql("""
CREATE TABLE IF NOT EXISTS raw_ecommerce.produtos (
    id_produto INT,
    id_categoria INT,
    nome_produto STRING,
    descricao STRING,
...
USING PARQUET
PARTITIONED BY (dt)
LOCATION 'hdfs://namenode:8020/data/raw/ecommerce/produtos'
""")

spark.sql("MSCK REPAIR TABLE raw_ecommerce.produtos")

print("✅ RAW produtos processada com sucesso")
