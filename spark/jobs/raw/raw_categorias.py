...
landing_base = "hdfs://namenode:8020/data/landing/ecommerce/categorias"
...
if not fs.exists(landing_path):
    print("⚠️ Nenhuma landing encontrada para categorias. Encerrando job.")
    spark.stop()
    exit(0)
...
if not datas_disponiveis:
    print("⚠️ Nenhuma partição dt encontrada na landing de categorias.")
    spark.stop()
    exit(0)
...
input_path = (
    ...
    f"categorias/dt={execution_date}"
)

output_base_path = (
    "hdfs://namenode:8020/data/raw/ecommerce/categorias"
)
...
df_raw = (
    df_landing
    .select(
        col("id_categoria").cast("int"),
        col("nome_categoria").cast("string"),
...
# Validação mínima
# -----------------------------
df_raw = df_raw.filter(col("id_categoria").isNotNull())
...
# -----------------------------
df_raw = df_raw.dropDuplicates(["id_categoria"])
...
spark.sql("""
CREATE TABLE IF NOT EXISTS raw_ecommerce.categorias (
    id_categoria INT,
...
USING PARQUET
PARTITIONED BY (dt)
LOCATION 'hdfs://namenode:8020/data/raw/ecommerce/categorias'
""")

spark.sql("MSCK REPAIR TABLE raw_ecommerce.categorias")

print("✅ RAW categorias processada com sucesso")
