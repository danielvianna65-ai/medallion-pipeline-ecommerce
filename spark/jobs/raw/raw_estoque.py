...
if not fs.exists(landing_path):
    print("⚠️ Nenhuma landing encontrada para estoque. Encerrando job.")
    spark.stop()
    exit(0)
...
if not datas_disponiveis:
    print("⚠️ Nenhuma partição dt encontrada para estoque.")
    spark.stop()
    exit(0)
...
# Paths
# -----------------------------
input_path = f"hdfs://namenode:8020/data/landing/ecommerce/estoque/dt={execution_date}"
output_base_path = "hdfs://namenode:8020/data/raw/ecommerce/estoque"
...
df_raw = (
    df_landing
    .select(
        col("id_estoque").cast("int"),
        col("id_produto").cast("int"),
        col("quantidade_disponivel").cast("int"),
...
# -----------------------------
df_raw = df_raw.dropDuplicates(["id_estoque"])
...
spark.sql("""
CREATE TABLE IF NOT EXISTS raw_ecommerce.estoque (
    id_estoque INT,
    id_produto INT,
...
USING PARQUET
PARTITIONED BY (dt)
LOCATION 'hdfs://namenode:8020/data/raw/ecommerce/estoque'
""")

spark.sql("MSCK REPAIR TABLE raw_ecommerce.estoque")

print("✅ RAW estoque processada com sucesso")
