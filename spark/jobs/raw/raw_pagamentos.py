...
landing_base = "hdfs://namenode:8020/data/landing/ecommerce/pagamentos"
...
input_path = (
    ...
    f"pagamentos/dt={execution_date}"
)

output_base_path = (
    "hdfs://namenode:8020/data/raw/ecommerce/pagamentos"
)
...
df_raw = (
    df_landing
    .select(
        col("id_pagamento").cast("int"),
        col("id_pedido").cast("int"),
        col("forma_pagamento").alias("metodo_pagamento"),
        col("status_pagamento").alias("status"),
        col("valor_pago").cast("decimal(12,2)").alias("valor"),
        col("data_pagamento").cast("timestamp"),
    )
)
...
USING PARQUET
PARTITIONED BY (dt)
LOCATION 'hdfs://namenode:8020/data/raw/ecommerce/pagamentos'

""")

spark.sql("MSCK REPAIR TABLE raw_ecommerce.pagamentos")
